...
func TestTopByScore(t *testing.T) {
	repo := NewRepository(setupTestDB(t))
	mirrors, err := repo.TopByScore(context.Background(), 2)
	if err != nil {
		t.Fatal(err)
	}
	if len(mirrors) != 2 {
		t.Fatalf("expected 2, got %d", len(mirrors))
	}
	if mirrors[0].URL != "https://mirror.de/archlinux/" {
		t.Errorf("expected best score first, got %q", mirrors[0].URL)
	}
...
func TestSelectByCountry(t *testing.T) {
	repo := NewRepository(setupTestDB(t))
	urls, err := repo.SelectByCountry(context.Background(), "DE", nil, 10)
	if err != nil {
		t.Fatal(err)
	}
	// Only HTTPS mirrors
	for _, u := range urls {
		if len(u) < 5 || u[:5] != "https" {
			t.Errorf("expected only https mirrors, got %q", u)
		}
	}
	// DE mirrors should come first
	if len(urls) < 1 || urls[0] != "https://mirror.de/archlinux/" {
		t.Errorf("expected DE mirror first, got %v", urls)
	}
}

func TestSelectByCountry_WithLastSync(t *testing.T) {
	repo := NewRepository(setupTestDB(t))
	lastSync := int64(1700000000)
	urls, err := repo.SelectByCountry(context.Background(), "US", &lastSync, 10)
	if err != nil {
		t.Fatal(err)
	}
	if len(urls) == 0 {
