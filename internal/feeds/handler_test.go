...
func setupTestDB(t *testing.T) *sql.DB {
	...
	for _, stmt := range []string{
		...
		`INSERT INTO release (version, available, info, release_date) VALUES
			('2024.01.01', 1, 'January release', 1704067200)`,
	} {
...
func TestNewsFeed(t *testing.T) {
	...
	mux.ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/news/feed", nil))
	...
	body := rr.Body.String()
	if !strings.Contains(body, "Test News") {
		t.Error("expected news title in feed")
	}
...
func TestPackagesFeed(t *testing.T) {
	db := setupTestDB(t)
	handler := NewHandler(news.NewRepository(db), packages.NewRepository(db), releases.NewRepository(db))
	...
	rr := httptest.NewRecorder()
	mux.ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/packages/feed", nil))
	...
	if !strings.Contains(body, "linux") {
		t.Error("expected package name in feed")
	}
}

func TestReleasesFeed(t *testing.T) {
	db := setupTestDB(t)
	handler := NewHandler(news.NewRepository(db), packages.NewRepository(db), releases.NewRepository(db))
	...
	rr := httptest.NewRecorder()
	mux.ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/releases/feed", nil))
	...
	if !strings.Contains(body, "2024.01.01") {
		t.Error("expected release version in feed")
	}
}

func TestNewsFeed_EmptyDB(t *testing.T) {
	db, err := sql.Open("sqlite", ":memory:")
	if err != nil {
		t.Fatal(err)
	}
	...
	if rr.Code != http.StatusOK {
		t.Errorf("expected 200 for empty feed, got %d", rr.Code)
	}
	if !strings.Contains(rr.Header().Get("Content-Type"), "atom+xml") {
		t.Error("expected atom content type")
	}
}
