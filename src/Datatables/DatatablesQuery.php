...
class DatatablesQuery
{
    ...
    private function configureQueryBuilder(
        ...
        if ($request->hasSearch() && $request->getSearch() !== null && !$request->getSearch()->isRegex()) {
        ...
        foreach ($request->getColumns() as $column) {
            if ($column->isSearchable()) {
                ...
                if (
                    $columnConfiguration->hasSearchableColumn($columnName)
                    && !$column->getSearch()->isRegex()
                    && $column->getSearch()->isValid()
                ) {
    ...
    /**
     * @param string[] $textSearchableColumns
     * @return string
     */
    private function createTextSearchQuery(array $textSearchableColumns): string
    {
     ...
     */
    private function createTextSearchesIterator(array $textSearchableColumns): \Iterator
    {
