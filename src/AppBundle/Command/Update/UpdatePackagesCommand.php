...
class UpdatePackagesCommand extends ContainerAwareCommand
{
        ...
        try {
            ...
            foreach ($this->getContainer()->getParameter('app.packages.repositories') as $repoName => $arches) {
                foreach ($arches as $arch) {
                    ...
                    $packageMTime = $this
                        ->entityManager
                        ->getRepository(\AppBundle\Entity\Packages\Package::class)
    ...
    private function updatePackage(Repository $repository, Package $databasePackage)
    {
        ...
        if (is_null($package)) {
            $package = \AppBundle\Entity\Packages\Package::createFromPackageDatabase($repository, $databasePackage);
        } else {
    ...
    private function cleanupObsoletePackages(Repository $repo, \DateTime $packageMTime, array $allPackages)
    {
        ...
        /** @var \AppBundle\Entity\Packages\Package $repoPackage */
