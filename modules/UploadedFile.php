		...
		if (isset($_FILES[$name]) && is_uploaded_file($_FILES[$name]['tmp_name'])) {
			...
			if (!$this->isAllowedType($this->file['type'])) {
				throw new FileException(sprintf($this->L10n->getText('Uploading files of type %s is not allowed'),
					htmlspecialchars($this->file['type'])));
			}
			if ($this->getFileSize() >= $this->Settings->getValue('file_size')) {
				throw new FileException(sprintf($this->L10n->getText('File is larger than %s Bytes'),
					$this->Settings->getValue('file_size')));
			}
		} elseif (isset($_FILES[$name]) && !empty($_FILES[$name]['error'])) // && !empty($_FILES[$name]['name']))
		{
			switch ($_FILES[$name]['error']) {
				case 1:
					$message = sprintf($this->L10n->getText('File is larger than %s Bytes'),
						ini_get('upload_max_filesize'));
				break;
				case 2:
					$message = $this->L10n->getText(
						'The uploaded file exceeds the directive that was specified in the form');
				break;
				case 3:
					$message = $this->L10n->getText('The uploaded file was only partially uploaded');
				break;
				case 4:
					$message = $this->L10n->getText('No file was uploaded');
				break;
				case 6:
					$message = $this->L10n->getText('Missing a temporary folder');
				break;
				case 7:
					$message = $this->L10n->getText('Failed to write file to disk');
				break;
				case 8:
					$message = $this->L10n->getText('File upload stopped by extension');
				break;
				default:
					$message = $this->L10n->getText('Unknown error. Code: ') . $_FILES[$name]['error'];
			}
